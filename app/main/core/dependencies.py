...
class OwnerTokenRequired(HTTPBearer):
    ...
    def __init__(self, token: Optional[str] = Query(None), roles=None, auto_error: bool = True):
        ...
        self.roles = roles
        self.token = token
        super(OwnerTokenRequired, self).__init__(auto_error=auto_error)

    async def __call__(self, request: Request, db: Session = Depends(get_db)):
        required_roles = self.roles
        credentials: HTTPAuthorizationCredentials = await super(OwnerTokenRequired, self).__call__(request)
