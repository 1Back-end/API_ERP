...
class CRUDFeature(CRUDBase[models.Feature,schemas.FeatureCreate,schemas.FeatureUpdate]):
    ...
    def get_many(
        ...
        *,
        ...
        record_query = db.query(models.Feature).filter(models.Feature.is_deleted == False)

        if keyword:
            record_query = record_query.filter(
                or_(
                    models.Feature.name.ilike(f"%{keyword}%"),
                )
            )

        if order and order_field and hasattr(models.Feature, order_field):
            if order.lower() == "asc":
                record_query = record_query.order_by(getattr(models.Feature, order_field).asc())
            else:
                record_query = record_query.order_by(getattr(models.Feature, order_field).desc())
        total = record_query.count()
        record_query = record_query.offset((page - 1) * per_page).limit(per_page).all()

        return schemas.FeaturesResponseList(
            total=total,
