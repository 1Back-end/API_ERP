...
async def login_owner(
    ...
    owner = crud.owner.authenticate(
        db=db, email=input.email, password=input.password
    )
    ...
    if not crud.owner.is_active(owner):
...
@router.get("/me/owner", summary="Get current user", response_model=schemas.OwnerProfile)
def get_current_user(
        current_user: any = Depends(TokenRequired()),
):
    """
...
@router.post("/start-reset-password/owner", response_model=schemas.Msg)
def start_reset_password(
        obj_in:schemas.ResetPasswordOption2Step1,
        db: Session = Depends(get_db),
...
@router.post("/check-otp-password/owner", summary="Check OTP password", response_model=schemas.Msg)
def check_otp_password(
        obj_in:schemas.ResetPasswordOption2Step2,
        db: Session = Depends(get_db),
) -> schemas.Msg:
...
@router.post("/reset-password/owner", summary="Reset password", response_model=schemas.Msg)
def reset_password(
        obj_in:schemas.ResetPasswordOption3Step3,
        db: Session = Depends(get_db),
) -> schemas.Msg:
